...
public class PutInActivity extends BaseActivity {
    ...
    @BindView(R.id.put_in_switch1)
    RelativeLayout putInSwitch;
    @BindView(R.id.ming_xi1)
    TextView mingXi;
    @BindView(R.id.hui_zong1)
    TextView huiZong;
    @BindView(R.id.tiao_ma1)
    TextView tiaoMa;
    @BindView(R.id.xing_hao1)
    TextView xingHao;
    @BindView(R.id.spq1)
    EditText spq;
    @BindView(R.id.ding_dan1)
    EditText dingDan;
    @BindView(R.id.ku_wei1)
    Spinner kuWei;
    @BindView(R.id.ku_fang1)
    Spinner kuFang;
    @BindView(R.id.put_in_activity_topbar)
    TopBar putInActivityTopbar;
    ...
    private void handleQueryByTiaoMaResult(Message msg) {
        ...
        if (!code.isEmpty() && "200".equals(code)) {
            ...
            if (vcModel != null && !vcModel.isEmpty()
                    && !spqStr.isEmpty() && null != lStoreId && null != lLocationId) {
                addRowAndUpdateUI();
            } else {
                tip("请填写完入库信息，再进行扫码");
            }
    ...
    private void handleAfterScanning(Message msg) {
        ...
        // 发送请求，根据条码查询连带数据
        mController.sendAsynMessage(IdiyMessage.QUERY_BY_TIAO_MA, scanResult, PUT_IN_STORE_MARK + "");
...
//                        addRowAndUpdateUI();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        View view = LayoutInflater.from(this).inflate(R.layout.activity_put_in, null);
...
//        initScanner();
    ...
    private void initFragment() {
        ...
        transaction.add(R.id.put_in_switch1, huiZongFragment, "right");
        transaction.add(R.id.put_in_switch1, mingXiFragment, "left");
        transaction.commit();
        // 明细显示，汇总隐藏
...
//        kuFang.setText(vcStoreName);
        ...
        putInActivityTopbar.setOnLeftAndRightClickListener(new TopBar.OnLeftAndRightClickListener() {
            @Override
            public void OnLeftButtonClick() {
                ActivityUtils.start(PutInActivity.this, MainActivity.class, false);
            }
            ...
            public void OnRightButtonClick() {
                ...
                proStore.setlStoreMark(PUT_IN_STORE_MARK);
                proStore.setProStoreDList(proStoreDList);
                // 保存入库明细
                mController.sendAsynMessage(IdiyMessage.INSERT_PUT_IN, proStore);
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            case IdiyMessage.QUERY_PRODUCT_1:
                if (null == data) {
                    return;
                }
...
//        }
     ...
     */
    @OnClick(R.id.xing_hao1)
    public void onTextChangedMingXi() {
        Intent intent = new Intent(PutInActivity.this, ProductQueryActivity.class);
        startActivityForResult(intent, IdiyMessage.QUERY_PRODUCT_1);
    }
